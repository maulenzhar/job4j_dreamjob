...
public class VacancyControllerTest {
    ...
    public void whenRequestVacancyGetById() {
        ...
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Integer.class);
        when(vacancyService.findById(vacancyArgumentCaptor.capture())).thenReturn(Optional.of(expectedVacancy));

        var model = new ConcurrentModel();
        var view = vacancyController.getById(model, expectedVacancy.getId());
        var actualVacancy = vacancyArgumentCaptor.getValue();

        assertThat(view).isEqualTo("vacancies/one");
        assertThat(actualVacancy).isEqualTo(expectedVacancy.getId());
    }

    @Test
    public void whenRequestVacancyUpdate() throws IOException {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenRequestVacancyDeleteById() {
        var expectedVacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Integer.class);
        when(vacancyService.deleteById(vacancyArgumentCaptor.capture())).thenReturn(true);

        var model = new ConcurrentModel();
        var view = vacancyController.delete(model, expectedVacancy.getId());
        var actualVacancy = vacancyArgumentCaptor.getValue();
